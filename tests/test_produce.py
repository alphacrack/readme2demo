...
def test_requested_formats_defaults_to_the_render_stage_set():
    ...
    both formats the render stage owns — so an untouched config reaches
    produce() with nothing left to do. This previously pinned a getattr
    bridge for "before #212 lands"; the bridge is retired, the guarantee is
    not.
    """
...
def test_requested_formats_empty_selection_falls_back():
    """Regression (#230): an explicitly emptied selection resolves to the
    render-stage set rather than silently dispatching nothing."""
