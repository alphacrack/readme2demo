...
Run directory layout (see IMPLEMENTATION_PLAN.md):
    ...
    runs/<run-id>/
      ...
      tutorial.md
      troubleshooting.md
      promo_script.json    # PromoScript — grounded scene plan for the promo cut
"""
...
class PromoScene(BaseModel):
    """One scene of the promo cut (#169).

    A ``demo_segment`` is REAL FOOTAGE: it names a step of the final
    ``step_by_step.md`` by ``step_index`` (an index into the ``steps`` array of
    ``step_timestamps.json``) plus the ``[start_s, end_s]`` span of that step to
    cut. That reference is what the grounding validator checks — a scene may
    only point at a step the fresh-container replay actually executed (see
    ``promo_script.collect_violations``).

    HONESTY, inherited from ``step_timestamps.json``: those offsets are LOWER
    BOUNDS on the TAPE clock, not exact positions in the rendered mp4 — the
    hidden preamble records no frames and VHS ``Wait`` time is excluded,
    shifting real footage right — so ``start_s``/``end_s`` are ESTIMATES that
    may land on different footage in the video. The step reference is the
    load-bearing fact; a compositor MUST resolve exact cut points against the
    real mp4 and keep the referenced step.

    ``title_card`` / ``end_card`` are presentation framing wrapped *around* the
    footage: they carry on-screen ``text`` drawn from run facts (repo name,
    the verified success command) and no video offsets. That text is gated too —
    a command printed on a card must be one the run can back
    (``promo_script.command_candidates``).
    """

    kind: Literal["title_card", "demo_segment", "end_card"]
    text: Optional[str] = None  # title/end cards ONLY: on-screen text (null on a segment)
    step_index: Optional[int] = None  # demo_segment: index into step_timestamps.json steps
    start_s: Optional[float] = None  # demo_segment: tape-clock ESTIMATE (lower bound), not an mp4 offset
    end_s: Optional[float] = None  # demo_segment: tape-clock ESTIMATE (lower bound), not an mp4 offset
    duration_s: float  # how long this scene runs


class PromoScript(BaseModel):
    """The promo cut's scene plan (promo_script.json) — consumed by #114 slice 2.

    Generated by one ``llm.complete_json`` call in ``promo_script.py`` and
    accepted only after a code-level validator confirms every ``demo_segment``
    traces to a verified step. No video is rendered here.
    """
